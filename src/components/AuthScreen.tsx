...
export default function AuthScreen() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-gradient-to-br from-gula-50 via-slate-50 to-emerald-50 px-4 dark:from-gula-950 dark:via-slate-950 dark:to-emerald-950">
      <button
        onClick={toggleTheme}
        className="absolute right-4 top-4 rounded-lg border border-slate-200 bg-white p-2 text-slate-500 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
      >
          ...
          <div className="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">
            {mode === "signin" ? (
              <>
                ...
                <button onClick={() => setMode("signup")} className="font-semibold text-gula-600 hover:underline dark:text-gula-400">
                  Sign up
                </button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => setMode("signin")} className="font-semibold text-gula-600 hover:underline dark:text-gula-400">
                  Sign in
                </button>
              </>
            )}
