  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-slate-500">What customers are saying about your shop</p>
      </div>
          ...
          <div className="mt-4 space-y-2">
            {ratingDist.map((d) => (
              <div key={d.star} className="flex items-center gap-2">
                <span className="flex w-4 items-center gap-0.5 text-xs text-slate-400">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white">
            ...
            {reviews.length === 0 ? (
              <div className="px-5 py-12 text-center">
                ...
                <p className="mt-1 text-xs text-slate-400">Reviews from your customers will appear here.</p>
              </div>
            ) : (
                  ...
                  <div key={r.id} className="px-5 py-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gula-50 text-sm font-semibold text-gula-700">
                          {r.buyer_name?.charAt(0).toUpperCase() ?? "?"}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-slate-900">{r.buyer_name}</p>
